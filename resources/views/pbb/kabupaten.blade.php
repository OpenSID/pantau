...
@section('content_header')
    <h1>Kabupaten PBB<small class="font-weight-light ml-1 text-md font-weight-bold">(Kabupaten yang memasang PBB ) @if($provinsi = session('provinsi')) {{ "| {$provinsi->nama_prov}" }} @endif</small></h1>
@stop
    ...
    var kabupaten = $('#table-versi').DataTable({
        ...
        columns: [
            ...
            },
            {
                orderable: false,
                data: function (data) {
                        return `<a target="_blank" href="{{ url('pbb/kecamatan') }}?kode_provinsi=${data.kode_provinsi}&kode_kabupaten=${data.kode_kabupaten}">${data.jumlah}</a>`
                    },
            },
